...
Here, no value is specified for R0, as the SYS call doesn't need to.  In BBC BASIC, R0 will be initialised to 0 before the
sys call is made.  This doesn't happen in Subtilis.  R0 is not modified by the compiler
before the call is made.   If you want to ensure that a register is set to 0 before
making a system call you must explicitly pass the value of 0 in the SYS argument list. BBC
BASIC programs also seem to get away without providing zero values for the trailing registers
used by a system call, so you need to be careful when porting programs that make SYS calls
from BBC BASIC to Subtilis.  Look up each system call and make sure you're passing all
the parameters it needs even if those parameters need to be set to 0.  If the registers
used by a system call contain unitialised data your program will behave in mysterious ways.
