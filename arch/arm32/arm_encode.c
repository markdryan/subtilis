...
static void prv_flush_pool(subtilis_arm_encode_ud_t *ud, subtilis_error_t *err)
{
	uint32_t word = 0;
...
static void prv_check_pool(subtilis_arm_encode_ud_t *ud, subtilis_error_t *err)
{
	...
	prv_flush_pool(ud, err);
}

static void prv_check_pool_ldrc(subtilis_arm_encode_ud_t *ud,
				subtilis_error_t *err)
{
	bool pool_needed = false;
	size_t pool_end = (ud->int_const_count + 1) +
			  (ud->real_const_count * 2) + ud->words_written + 2;
				  ...
				  subtilis_error_t *err)
{
	...
	/*
	 * So this is a little tricky.  Loading constants with lca
	 * involves two instructions, an LDR and an ADD, both of which
	 * use R15 as a source.  It's important that they are not split
	 * up by a constant pool.  So here we use a modified version of
	 * prv_check_pool that will flush the constant pool if there
	 * isn't enough space for 1 integer constant and 2 instructions.
	 * We need to do this check for standalone ldrc instructions as
	 * well.  Right now we don't have a way of detecting a standlone
	 * instruction.  However, the current  scheme works for floating
	 * point constants, and flushes the pool 4 bytes too early for
	 * standalone ints.
	 *
	 * TODO, this needs tidying up.  It's just
	 * luck it works for floats and we're possibly making our binary
	 * bigger when dealing with ints.  To fix we need a way to tell
	 * whether an LDRC instruction is generated by LCA or not.
	 */

	prv_check_pool_ldrc(ud, err);
	if (err->type != SUBTILIS_ERROR_OK)
		return;
