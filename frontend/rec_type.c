...
static void prv_init_array(subtilis_parser_t *p, const subtilis_type_t *type,
			   const char *var_name, size_t mem_reg, size_t loc,
			   subtilis_error_t *err)
{
	size_t i;
...
static void prv_init_vector(subtilis_parser_t *p, const subtilis_type_t *type,
			    size_t mem_reg, size_t loc, int32_t vec_dim,
			    subtilis_error_t *err)
{
	subtilis_ir_operand_t op;
...
static void prv_init_field(subtilis_parser_t *p, const subtilis_type_t *type,
			   subtilis_type_field_t *field, size_t mem_reg,
			   size_t loc, bool memset, subtilis_error_t *err)
{
	subtilis_exp_t *e;

	if (subtilis_type_if_is_array(type)) {
		prv_init_array(p, type, field->name, mem_reg, loc, err);
	} else if (subtilis_type_if_is_vector(type) && (field->vec_dim >= 0)) {
		prv_init_vector(p, type, mem_reg, loc, field->vec_dim, err);
			      ...
			      size_t mem_reg, size_t loc, subtilis_error_t *err)
{
	...
	for (i = 0; i < rec->num_fields; i++) {
		...
		prv_init_field(p, field, &rec->fields[i], mem_reg, offset + loc,
			       true, err);
		if (err->type != SUBTILIS_ERROR_OK)
			return;
	}
