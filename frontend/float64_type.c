				    ...
				    size_t ptr, subtilis_error_t *err)
{
	...
	reg = subtilis_ir_section_add_real_call_ptr(p->current, num_args, args,
						    ptr, err);
	if (err->type != SUBTILIS_ERROR_OK)
		return NULL;
			     ...
			     size_t reg1, size_t reg2, subtilis_error_t *err)
{
	...
	tmp.reg = subtilis_ir_section_add_instr2(
	    p->current, SUBTILIS_OP_INSTR_MOVFP, op1, err);
	if (err->type != SUBTILIS_ERROR_OK)
		return;

	subtilis_ir_section_add_instr_no_reg2(
	    p->current, SUBTILIS_OP_INSTR_MOVFP, op1, op2, err);
	if (err->type != SUBTILIS_ERROR_OK)
		return;

	subtilis_ir_section_add_instr_no_reg2(
	    p->current, SUBTILIS_OP_INSTR_MOVFP, op2, tmp, err);
}
			     ...
			     size_t reg1, size_t reg2, subtilis_error_t *err)
{
	...
	subtilis_ir_section_add_instr_reg(
	    p->current, SUBTILIS_OP_INSTR_LOADO_REAL, op1, op2, zero, err);
	if (err->type != SUBTILIS_ERROR_OK)
		return;

	subtilis_ir_section_add_instr_reg(
	    p->current, SUBTILIS_OP_INSTR_STOREO_REAL, tmp, op2, zero, err);
}
			     ...
			     size_t reg1, size_t reg2, subtilis_error_t *err)
{
	...
	tmp.reg = subtilis_ir_section_add_instr(
	    p->current, SUBTILIS_OP_INSTR_LOADO_REAL, op1, zero, err);
	if (err->type != SUBTILIS_ERROR_OK)
		return;

	tmp2.reg = subtilis_ir_section_add_instr(
	    p->current, SUBTILIS_OP_INSTR_LOADO_REAL, op2, zero, err);
	if (err->type != SUBTILIS_ERROR_OK)
		return;
	...
	subtilis_ir_section_add_instr_reg(
	    p->current, SUBTILIS_OP_INSTR_STOREO_REAL, tmp2, op1, zero, err);
}
